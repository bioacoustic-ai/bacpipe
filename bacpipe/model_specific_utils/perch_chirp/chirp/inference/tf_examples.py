...
def get_feature_description(logit_names: Sequence[str] | None = None):
    ...
    feature_description = {
        ...
        FRONTEND_SHAPE: tf.io.FixedLenSequenceFeature([], tf.int64, allow_missing=True),
    ...
    if logit_names is not None:
        for logit_name in logit_names:
            ...
            feature_description[f"{logit_name}_shape"] = tf.io.FixedLenSequenceFeature(
                [], tf.int64, allow_missing=True
            )
...
def create_embeddings_dataset(
    ...
    ds = tf.data.TFRecordDataset(embeddings_files, num_parallel_reads=tf.data.AUTOTUNE)

    parser = get_example_parser(logit_names=logit_names, tensor_dtype=tensor_dtype)
...
def model_outputs_to_tf_example(
    ...
    if model_outputs.logits is not None and write_logits:
        ...
        for logits_key in logit_keys:
            ...
            feature[logits_key] = bytes_feature(serialize_tensor(logits, tensor_dtype))
