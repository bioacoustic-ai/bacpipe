    ...
    if domain is Domain.TIME:
        ...
        def _morlet_wavelet(f: jnp.ndarray) -> jnp.ndarray:
            ...
            return (
                ...
                * jnp.pi ** (1 / 4)
                * jnp.sqrt(2)
                * (jnp.exp(-1 / 2 * (sigma - f) ** 2) - kappa * jnp.exp(-1 / 2 * f**2))
            )
...
def melspec_params(
    ...
    range_ = map(
        chirp.signal.hertz_to_mel, (lower_edge_hertz, upper_edge_hertz)
...
def multiply_filter(
    ...
    *_, num_frames, _ = signal.shape
    ...
    filtered_signal = jnp.fft.ifft(jnp.fft.fft(signal, axis=-2) * filter_(fs), axis=-2)
    if normalization is Normalization.L1:
        norm = 1
    elif normalization is Normalization.L2:
