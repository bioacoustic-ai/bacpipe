...
SAVE_PATH = Path(config["audio_dir"]).joinpath(Path(config_preproc["annots_path"]).stem)
...
def get_corresponding_sound_file(file):
    ...
    file_path = glob.glob(
        f'{hard_drive_path}/**/{Path(file).stem.split("Table")[0]}wav', recursive=True
    )

    if not file_path:
        new_file = Path(file).stem.split("Table")[0] + "wav"
        file_tolsta = "336097327." + new_file[6:].replace("_000", "").replace("_", "")
        file_path = glob.glob(f"{hard_drive_path}/**/{file_tolsta}", recursive=True)
...
def get_number_of_segs_per_call(annots):
    ...
    """
    num_of_segs_per_call = np.round(
        ((annots.end - annots.start) / CORRECTED_CONTEXT_WIN_TIME), 0
    ).astype(int)
...
def extract_segments(file):
    ...
    """
    call_len = config["preproc"]["model_sr"] * CORRECTED_CONTEXT_WIN_TIME
    n = int(config["segs_lim"] * call_len)
    win_length = int(config_preproc["model_sr"] * 1024 / config_preproc["model_sr"])
    ...
    calls_1d_all, segs_per_call = create_1d_call_array(segs_per_call, annots, audio)

    if len(calls_1d_all) >= n:
        calls_1d_temp = [
            calls_1d_all[i : i + n] for i in range(0, len(calls_1d_all), n)
        ]
        calls_1d_list = []
...
def init_call_array(num_of_segs_per_call):
    ...
    return np.zeros(
        [
            num_of_segs_per_call.sum() + 1,
            int(CORRECTED_CONTEXT_WIN_TIME * config["preproc"]["model_sr"]),
        ]
    )
...
def get_segment_indices(annots, row, seg_num):
    ...
    """
    beg = int(
        (row.start - annots.start.values[0] + CORRECTED_CONTEXT_WIN_TIME * seg_num)
        * config["preproc"]["model_sr"]
    )

    end = int(beg + CORRECTED_CONTEXT_WIN_TIME * config["preproc"]["model_sr"])
    return beg, end
...
def extend_df(df_singles, segs_per_call):
    ...
    df_repeated = pd.DataFrame(
        np.repeat(df_singles.to_numpy(), segs_per_call, axis=0),
...
def condense_files_into_only_calls():
    ...
    annotation_files = glob.glob(
        config["preproc"]["annots_path"] + "/**/*.txt", recursive=True
    )
