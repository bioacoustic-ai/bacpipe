...
class Model(ModelBaseClass):
    def __init__(self):
        ...
        self.mel = T.MelSpectrogram(sample_rate=SAMPLE_RATE, 
                                    n_fft=NFFT, 
                                    hop_length=HOPLEN, 
                                    f_min=FMIN, 
                                    f_max=FMAX, 
                                    n_mels=NMELS).to(DEVICE)
        self.power_to_db = T.AmplitudeToDB()
        self.norm = Normalization()

        self.model = cvt13()
        state_dict = torch.load('bacpipe/models/protoclr/protoclr_300.pth', map_location='cpu')
