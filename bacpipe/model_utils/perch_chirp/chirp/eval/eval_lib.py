...
def _prepare_eval_set(
    ...
    num_representatives_per_class = eval_set_specification.num_representatives_per_class
    ...
    for class_name in eval_set_specification.class_names:
        ...
        class_representative_mask = global_class_representative_mask & _df_eval(
            embeddings_df,
            eval_set_specification.class_representative_classwise_mask_fn(class_name),
        )
        ...
        if (
            ...
            | search_corpus_df["bg_labels"][search_corpus_mask].str.contains(class_name)
        ).sum() == 0:
            logging.warning(
...
def prepare_eval_sets(
    ...
    logging.info("Preparing %d unique eval sets.", len(config.eval_set_specifications))
