...
class Pipeline:
    ...
    @staticmethod
    def chain(ops: Sequence[FeaturesPreprocessOp], dataset_info: tfds.core.DatasetInfo):
...
class RandomNormalizeAudio(FeaturesPreprocessOp):
    ...
    def __call__(
        ...
        target_gain = tf.random.uniform([], minval=self.min_gain, maxval=self.max_gain)
        return NormalizeAudio(target_gain=target_gain, names=self.names, eps=self.eps)(
...
class MixAudio(DatasetPreprocessOp):
    ...
    def _reduce_func(self, key: tf.Tensor, dataset: tf.data.Dataset) -> tf.data.Dataset:
...
class ConvertReefLabels(FeaturesPreprocessOp):
    ...
    def map_and_encode(
        ...
        output_features.update({"reef_label": output_labels, "reef_label_mask": mask})
...
class OnlyKeep(FeaturesPreprocessOp):
    ...
    def __call__(
        ...
        return {
            name: feature for name, feature in features.items() if name in self.names
        }
        ...
        def map_fn(example):
            ...
            if self.drop_annotation_bounds:
                ...
                for k in (
                    ...
                    example[k] = tf.cast(
                        tf.gather(example[k], overlap_indices), tf.int64
                    )
