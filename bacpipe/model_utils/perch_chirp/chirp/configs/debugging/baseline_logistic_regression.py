...
from bacpipe.model_utils.perch_chirp.chirp.configs.debugging import presets
...
def get_pipeline_ops(
    ...
    if random_normalize:
        normalize_op = _c("pipeline.RandomNormalizeAudio", min_gain=0.15, max_gain=0.25)
    else:
...
def get_config() -> config_dict.ConfigDict:
    ...
    num_train = config.get_ref("train_window_size_s") * config.get_ref("sample_rate_hz")
    num_eval = config.get_ref("eval_window_size_s") * config.get_ref("sample_rate_hz")
    ...
    num_train_frames = (num_train + stride - odd_kernel) // stride + odd_kernel - 1
