...
def refresh_codebooks(
    ...
    for k, codebook in flat_params.items():
        ...
        idxs = jax.random.categorical(categorical_key, counts, shape=[num_centroids])
...
class BaseQuantizer(nn.Module):
    ...
    def create_codebook(self, flat_inputs):
        ...
        codebook = self.param("codebook", init_fn, (self.num_centroids, embedding_dim))
        return codebook
...
class ProductQuantizer(nn.Module):
    ...
    def get_pca_layer(self, embedding_dim):
        ...
        pre_bias = self.param("pre_bias", jax.nn.initializers.zeros, [1, embedding_dim])
    ...
    def __call__(self, inputs, train):
        ...
        return QuantizerOutputs(quantized, quantization_loss, nn_idx, codebook, counts)
...
class ResidualQuantizer(nn.Module):
    ...
    def __call__(self, inputs, train=True):
        ...
        quantized = jnp.reshape(quantized, inputs.shape)
        quantization_loss = jnp.full(inputs.shape[:-1] + (1,), quantization_loss)
        return QuantizerOutputs(quantized, quantization_loss, nn_idx, codebooks, counts)
