...
class TransformerFeedForward(nn.Module):
    ...
    def __call__(self, inputs: jnp.ndarray, train: bool) -> jnp.ndarray:
        ...
        projected_inputs = FeedForward(output_dims=output_dims, activation=Identity())(
            projected_inputs
        )
...
class LightConv1D(nn.Module):
    ...
    def __call__(
        ...
        act_inputs = FeedForward(output_dims=self.input_dims, activation=Identity())(
            inputs
        )
        gated_inputs = FeedForward(output_dims=self.input_dims, activation=Identity())(
            inputs
        )
        ...
        inputs = FeedForward(output_dims=self.input_dims, activation=Identity())(inputs)
...
class Conformer(nn.Module):
    ...
    def __call__(
        ...
        if self.layer_order not in layer_order_set:
            raise ValueError(f"`self.layer_order` must be within `{layer_order_set}`.")
        ...
        if self.layer_order == "mhsa":
            ...
            inputs = lconv(inputs, train=train, use_running_average=use_running_average)
...
class StridedAutopool(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        alpha = self.param("alpha", nn.initializers.constant(self.alpha_0), alpha_shape)
