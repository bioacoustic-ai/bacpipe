...
def compute_mask_indices(
    ...
    mask_idc = jnp.concatenate([mask_idc + offset for offset in range(mask_length)])
    ...
    batch_inds = jnp.reshape(
        jnp.repeat(jnp.expand_dims(jnp.arange(bsz), 1), axis=1, repeats=max_masks),
        [-1],
    )
    batch_inds = jnp.reshape(
...
class HuBERTModel(nn.Module):
    ...
    def add_projected_quantizer(self, x, quantizers, train):
        ...
        codes_pj = [
            nn.Dense(self.final_dim, name="codes_proj_{}_{}".format(quant_index, i))(
                codes[i]
            )
            for i in range(ns)
        ]
        ...
        quantizers.append(
            QuantizerBundle(quant_outputs.quantization_loss, targets, codes, codes_pj)
        )
        return quantizers
    ...
    def __call__(
        ...
        # [bsz, sz, nc].
        quant_losses = [quantizers[i].quantization_loss for i in range(len(quantizers))]
