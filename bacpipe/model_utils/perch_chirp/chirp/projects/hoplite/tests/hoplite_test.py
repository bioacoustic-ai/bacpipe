...
class HopliteTest(parameterized.TestCase):
    ...
    def test_labels_db_interface(self, db_type):
        ...
        db.insert_label(
            interface.Label(ids[0], "hawgoo", interface.LabelType.POSITIVE, "machine")
        )
        db.insert_label(
            interface.Label(ids[1], "hawgoo", interface.LabelType.POSITIVE, "machine")
        )
        db.insert_label(
            interface.Label(ids[0], "rewbla", interface.LabelType.NEGATIVE, "machine")
        )
    ...
    def test_brute_search_impl_agreement(self):
        ...
        sqlite_db = test_utils.make_db(self.tempdir, "sqlite", 0, rng, EMBEDDING_SIZE)
