...
def update_delegates(
    ...
    stacked_candidates = jnp.tile(candidates[jnp.newaxis, :], [candidates.shape[0], 1])
    ...
    combined_scores = jnp.where(combined_delegates == -1, -jnp.inf, combined_scores)

    # Sort the combined delegates by score.
    combined_scores, combined_delegates = (
        cosort(  # pylint: disable=unbalanced-tuple-unpacking
...
def multi_brute_search(queries: jnp.ndarray, embeddings: jnp.ndarray, top_k: int):
...
def make_delegate_lists(
    ...
    delegate_scores = -jnp.inf * jnp.ones([num_embeddings, max_delegates], jnp.float16)
