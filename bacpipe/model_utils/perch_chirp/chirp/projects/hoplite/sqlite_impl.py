...
class SQLiteGraphSearchDB(interface.GraphSearchDBInterface):
    ...
    def insert_edges(self, x_id: int, y_ids: np.ndarray, replace: bool = False) -> None:
        cursor = self._get_cursor()
        if not replace:
    ...
    def get_embedding_source(self, embedding_id: int) -> interface.EmbeddingSource:
        cursor = self._get_cursor()
        cursor.execute(
            """
    ...
    def get_embeddings(
        ...
        cursor = self._get_cursor()
        results = cursor.execute(query, tuple(int(c) for c in embedding_ids)).fetchall()
        result_ids = np.array(tuple(int(c[0]) for c in results))
        embeddings = np.array(
            tuple(deserialize_embedding(c[1], self.embedding_dtype) for c in results)
        )
...
def serialize_embedding(embedding: np.ndarray, embedding_dtype: type[Any]) -> bytes:
