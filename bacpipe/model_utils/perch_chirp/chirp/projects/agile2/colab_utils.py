    ...
    if "model_config" in db_metadata:
        if db_metadata["model_config"].model_key != model_config.model_key:
            raise AssertionError(
                ...
                " use the '%s' model confg." % db_metadata["model_config"].model_key
            )

    db.insert_metadata("model_config", model_config.to_config_dict())
    db.insert_metadata("embed_config", configs.audio_sources_config.to_config_dict())
    db.commit()
...
def load_configs(
    ...
    if db_path is None:
        ...
        db_path = epath.Path(next(iter(audio_globs.values()))[0]) / "hoplite_db.sqlite"
