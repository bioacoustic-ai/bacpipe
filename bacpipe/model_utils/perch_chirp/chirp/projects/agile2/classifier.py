...
def forward(
    ...
    weights = batch.is_labeled_mask + (1.0 - batch.is_labeled_mask) * weak_neg_weight
    # Loss shape is [B, C]
    if loss_name == "hinge":
...
def eval_classifier(
    ...
    rocs = metrics.roc_auc(logits=pred_logits, labels=true_labels, sample_threshold=1)
    cmaps = metrics.cmap(logits=pred_logits, labels=true_labels, sample_threshold=1)
    return {
        "top1_acc": top1,
        "roc_auc": rocs["macro"],
