...
class ZooTest(parameterized.TestCase):
    ...
    def test_pooled_embeddings(self):
        ...
        for pooling_method in zoo_interface.POOLING_METHODS:
            ...
            batched_time_pooled = batched_outputs.pooled_embeddings(pooling_method, "")
    ...
    def test_taxonomy_model_tf(self, model_return_type, batchable):
        ...
        class_list = {"label": namespace.ClassList("fake", ["alpha", "beta", "delta"])}
        ...
        outputs = wrapped_model.batch_embed(np.zeros([10, 5 * 32000], dtype=np.float32))
        ...
        outputs = wrapped_model.batch_embed(np.zeros([2, 20 * 32000], dtype=np.float32))
        self.assertTrue(outputs.batched)
    ...
    def test_whale_model(self):
        ...
        with self.subTest("from_config"):
            ...
            # verify
            self.assertFalse(outputs.batched)
            self.assertSequenceEqual(outputs.embeddings.shape, [expected_frames, 1, 32])
            self.assertSequenceEqual(
