...
from bacpipe.model_utils.perch_chirp.chirp.models import mae
...
def initialize_finetune_model(
...
) -> tuple[classifier.train_utils.ModelBundle, classifier.train_utils.TrainState]:
    """Creates model for training, eval, or inference."""
    # Initialize random number generator
    key = random.PRNGKey(rng_seed)
    ...
    # Load checkpoint
    mae_model_bundle, mae_train_state = initialize_model(**model_config.mae_init_config)
    ...
    def forward(params, key, batch, model_state):
        ...
        loss = (
            jnp.mean(
                jnp.sum(
                    (
                        jnp.take_along_axis(patches, masked[..., jnp.newaxis], axis=1)
                        - jnp.take_along_axis(
        ...
        reconstructed = jnp.reshape(decoded_patches, (b, h // ph, w // pw, ph, pw, c))
        reconstructed = jnp.reshape(jnp.swapaxes(reconstructed, -3, -4), (b, h, w, c))
        images = {
