...
def initialize_model(
    ...
    num_classes = {md.key: len(md.class_list.classes) for md in output_head_metadatas}
...
def evaluate(
    ...
    metrics_["rank_metrics"] = train_utils.CollectingMetrics.from_funs(**valid_metrics)
                ...
                if batch["label"].ndim == 2:
                    ...
                    if even_batch["label"].shape[1] > 0:
                        new_valid_metrics = get_metrics(
                            even_batch, replicated_train_state
                        )
