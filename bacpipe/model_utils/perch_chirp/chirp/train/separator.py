...
def initialize_model(
    ...
    variables = model.init(model_init_key, jnp.zeros((1,) + input_shape), train=False)
...
def _audio_and_spectrogram_summaries(
    ...
    mixes = np.reshape(batch["source_audio"], (-1, *batch["source_audio"].shape[2:]))
    separated_audio = np.reshape(separated_audio, (-1, *separated_audio.shape[-2:]))
    ...
    for i in range(min(batch_size, max_outputs)):
        fig, axes = plt.subplots(n_rows, 2, figsize=(2 * img_size, img_size * n_rows))
...
def train(
    ...
    train_metrics_collection = train_utils.NestedCollection.create(**TRAIN_METRICS)
    ...
    def train_step(batch, train_state):
        ...
        updates, opt_state = model_bundle.optimizer.update(grads, train_state.opt_state)
...
def evaluate(
    ...
    def get_metrics(batch, train_state):
        ...
        mixit_neg_snr = p_log_snr_loss(batch["source_audio"], estimate, loss_max_snr)
...
def run(
    ...
    model_bundle, train_state = initialize_model(workdir=workdir, **config.init_config)
    if mode == "train":
