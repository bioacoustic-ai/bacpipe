...
from bacpipe.model_utils.perch_chirp.chirp.models import frontend as frontend_models
...
def quantizer_loss(
    ...
    quant_loss = jnp.squeeze(
        jnp.mean(quant_loss, -1)
...
def get_train_metrics(
    ...
    metrics_ = {
        ...
        "supervised_loss": clu_metrics.Average.from_fun(
            functools.partial(supervised_loss, readout_loss_mult=readout_loss_mult)
        ),
    }
...
def initialize_model(
    ...
    **unused_kwargs,
    ...
    if reload_quantizer_from:
        ...
        train_state.params[quantizer_key] = reloaded_quantizer[
            "params"
        ][  # pytype: disable=unsupported-operands  # py310-upgrade
            quantizer_key
        ]
    ...
    if reload_hubert_from:
        ...
        if reloaded_hubert is None:
            raise RuntimeError("Unable to reload HuBERT from %s." % reload_hubert_from)
        logging.info(
    ...
    return (
        train_utils.ModelBundle(model=model, key=key, ckpt=ckpt, optimizer=optimizer),
        train_state,
        learning_rate,
    )
...
def evaluate(
    ...
    metrics_["rank_metrics"] = train_utils.CollectingMetrics.from_funs(**rank_metrics)
    ...
    def get_metrics(batch, train_state, mask_key):
        ...
        mutable = list(train_state.model_state.keys()) if train_mode_at_eval else False
    ...
    for train_state in train_utils.checkpoint_iterator(
        ...
        with reporter.timed("eval"):
            ...
            # Log validation loss
            train_utils.write_metrics(writer, step, valid_metrics.compute(prefix=name))
        writer.flush()
