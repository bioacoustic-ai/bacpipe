...
def load_audio_file(
    ...
    with tempfile.NamedTemporaryFile(mode="w+b", suffix=extension, delete=False) as f:
...
def load_xc_audio(xc_id: str, sample_rate: int, dtype: str = "float32") -> jnp.ndarray:
...
def load_url_audio(url: str, sample_rate: int, dtype: str = "float32") -> jnp.ndarray:
...
def random_low_pass_filter(
    ...
    offset = random.uniform(offset_key, shape, minval=min_offset, maxval=max_offset)
