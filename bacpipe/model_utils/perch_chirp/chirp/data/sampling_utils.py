    ...
    def _subsample(group_df):
        ...
        indices = sorted(
            jax.random.choice(
                ...
                jax.random.fold_in(
                    key,
                    int(hashlib.md5(species_code.encode()).hexdigest()[:8], base=16),
                ),
                len(group_df),
...
def valid_recording(
    ...
    if (remaining_fg[current_species] > 0 and recording[0] == current_species) or (
        remaining_bg[current_species] > 0 and current_species in recording[1]
    ):
        # Ensure it doesn't violate any constraint.
        violates_fg = recording[0] in remaining_fg and remaining_fg[recording[0]] == 0
        violates_bg = any(
