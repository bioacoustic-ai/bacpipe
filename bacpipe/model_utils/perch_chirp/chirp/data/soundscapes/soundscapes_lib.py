...
MetadataLoaderType = Callable[[epath.Path, dict[str, MetadataFeature]], pd.DataFrame]
...
def load_class_list(
    ...
    if keep_unknown_annotation and UNKNOWN_LABEL not in dataset_class_list.classes:
...
def add_annotated_urls(
    ...
    segments = segments[
        segments["url"].apply(lambda url: url != "")
...
def get_full_length_annotations(
    ...
    malformed_segment = annotations["annotation_end"] < annotations["annotation_start"]
