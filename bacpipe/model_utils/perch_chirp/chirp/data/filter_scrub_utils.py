...
def apply_complement(
    ...
    updated_df = APPLY_FN[type(query_complement.query)](df, query_complement.query)
...
def apply_sequence(
    ...
    if query_sequence.mask_query is not None:
        mask = APPLY_FN[type(query_sequence.mask_query)](df, query_sequence.mask_query)
...
def is_in(df: pd.DataFrame, key: str, values: list[SerializableType]) -> pd.Series:
...
def contains_any(df: pd.DataFrame, key: str, values: list[str]) -> pd.Series:
    ...
    values_types = set(type(v) for v in values)
    df_column_types = set().union(*df[key].map(lambda xs: set(type(x) for x in xs)))
    if len(values_types.union(df_column_types)) != 1:
...
def is_not_in(df: pd.DataFrame, key: str, values: list[SerializableType]) -> pd.Series:
...
def scrub(
    ...
    if (
        len(values) == 0 or len(feature_dict[key]) == 0
