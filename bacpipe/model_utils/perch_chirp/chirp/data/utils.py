...
def get_multi_dataset(
    ...
    if tfds_data_dirs is not None and len(tfds_data_dirs) != num_datasets:
        raise ValueError("Length of tfds_data_dirs does not match number of datasets.")
    ...
    for dataset_dir, ds_pipeline, tfds_data_dir in zip(
        ...
        ds, dataset_info = get_base_dataset(split, is_train, dataset_dir, tfds_data_dir)
    ...
    unified_ds = tf.data.Dataset.sample_from_datasets(merge_datasets, weights=weights)
