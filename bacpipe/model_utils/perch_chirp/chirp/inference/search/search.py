...
def _random_sort_scores(ex):
    ex["sort_scores"] = tf.random.uniform([tf.shape(ex[tf_examples.EMBEDDING])[0]])
    ex["max_sort_score"] = tf.reduce_max(ex["sort_scores"])
    return ex
...
def search_embeddings_parallel(
    ...
    score_fn: (
        Callable[[Any, np.ndarray], Any] | str
    ) = "euclidean",  # pylint: disable=g-bare-generic
    ...
    score_fn = functools.partial(score_fn, query_embedding_batch=query_embedding_batch)
    if random_sample:
...
def classifer_search_embeddings_parallel(
    ...
    **kwargs,
    ...
    def classify_batch(batch, query_embedding_batch):
        ...
        logits = tf.reshape(logits, [emb_shape[0], emb_shape[1], tf.shape(logits)[-1]])
