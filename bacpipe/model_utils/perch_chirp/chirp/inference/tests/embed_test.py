...
def _make_output_head_model(model_path: str, embedding_dim: int = 1280):
    ...
    return zoo_interface.LogitsOutputHead(model_path, "other_label", model, class_list)
...
class EmbedTest(parameterized.TestCase):
    ...
    def test_keyed_write_logits(self):
        ...
        parser = tf_examples.get_example_parser(logit_names=["label", "other_label"])
        got_example = parser(serialized)
        self.assertIsNotNone(got_example)
        self.assertEqual(got_example[tf_examples.FILE_NAME], "clap.wav")
        self.assertSequenceEqual(
    ...
    def test_logits_output_head(self):
        ...
        logits_model = _make_output_head_model("/tmp/logits_model", embedding_dim=128)
        ...
        reupdated_outputs = restored_model.add_logits(base_outputs, keep_original=True)
        error = np.mean(
            np.abs(
    ...
    def test_create_source_infos(self):
        ...
        globs = [
            path_utils.get_absolute_path("inference/tests/testdata/clap.wav").as_posix()
        ]
    ...
    def test_tfrecord_multiwriter(self):
        ...
        with tf_examples.EmbeddingsTFRecordMultiWriter(output_dir.as_posix()) as writer:
