...
def get_existing_source_ids(
    ...
    for e in dataset.as_numpy_iterator():
        existing_source_ids.add(SourceId(str(e["filename"], "UTF_8"), e["timestamp_s"]))
...
class EmbedFn(beam.DoFn):
    ...
    def process(self, source_info: SourceInfo, crop_s: float = -1.0):
        ...
        logging.info("...creating embeddings (%s / %d)", file_id, timestamp_offset_s)
...
def get_config(config_key: str, shard_idx: str = "") -> config_dict.ConfigDict:
    ...
    if shard_idx:
        config = importlib.import_module(module_key, INFERENCE_CONFIGS_PKG).get_config(
            shard_idx
        )
    else:
        config = importlib.import_module(module_key, INFERENCE_CONFIGS_PKG).get_config()
