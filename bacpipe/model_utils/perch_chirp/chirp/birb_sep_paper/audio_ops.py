...
from bacpipe.model_utils.perch_chirp.chirp.birb_sep_paper import pcen_ops
import numpy as np
...
def CleanupMelspec(melspec, feature_cleaning):
    ...
    if feature_cleaning["strategy"] == "whiten":
        melspec = TwoStageWhitening(melspec, thresh=feature_cleaning["clean_thresh"])
        ...
        raise ValueError("Unknown feature cleaning strategy : %s" % feature_cleaning)
    return melspec
...
def TwoStageWhitening(batched_melspec, thresh=0.5):
    ...
    masked_std = tf.reduce_sum(mask * tf.square(batched_melspec - masked_mean), axis=1)
...
def ApplySpecAugmentMask(target, axis, min_length=0.0, max_length=0.5):
    ...
    def RandRoll(x):
        return tf.roll(
            x,
            tf.random.uniform([], minval=0, maxval=target.shape[axis], dtype=tf.int64),
            axis=0,
        )
...
def ApplySpecAugment(raw_melspec, batched=False, **kwargs):
    ...
    if "specaug_max_freq_mask" in kwargs:
        ...
        max_length = None
    melspec = ApplySpecAugmentMask(melspec, 2, min_length=0.0, max_length=max_length)
    melspec = ApplySpecAugmentMask(melspec, 1, min_length=0.0, max_length=max_length)
    if batched:
        return melspec
    else:
        return melspec[0, :, :]
