...
flags.DEFINE_string("model_path", "", "Where to find the model params and inference.pb")
flags.DEFINE_string(
...
class EmbedFn(beam.DoFn):
    ...
    def setup(self):
        ...
        classifiers = model_utils.load_classifier_ensemble(self.model_path, max_runs=1)
...
def get_counter(metrics, name):
    counter = metrics.query(beam.metrics.MetricsFilter().with_name(name))["counters"]
    if not counter:
        return 0
