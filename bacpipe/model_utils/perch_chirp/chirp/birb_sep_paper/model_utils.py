...
def map_k(labels_onehot, logits, k=1, name=""):
    ...
    class_ids = tf.expand_dims(tf.range(labels_onehot.shape[-1], dtype=tf.int64), 0)
    masked_class_ids = labels_onehot * class_ids
    # Set the false labels to -1, since the zero label is allowed.
    masked_class_ids += (labels_onehot - 1) * tf.ones(labels_onehot.shape, tf.int64)
...
def load_classifier_state(model_path, sample_rate=22050):
    ...
    with cl_graph.as_default():
        ...
        if "hints_input" in signature_def.inputs:
            ...
            hints_input = None
        all_outputs = {k: signature_def.outputs[k].name for k in signature_def.outputs}
...
def ensemble_classify(audio_batch, classifier_states, hints=None, logits_key=None):
...
def separate_classify(
    ...
    big_batch = np.reshape(sep_chunks, [sep_chunks.shape[0] * sep_chunks.shape[1], -1])
...
def make_eval_metrics(mode_key, model_dir, eval_dict):
    ...
    tensor_map["global_step"] = tf.expand_dims(tf.train.get_or_create_global_step(), 0)
