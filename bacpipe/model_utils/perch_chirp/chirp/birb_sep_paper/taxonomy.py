...
class Taxonomy(object):
    ...
    def GenerateEnums(self, code_list, whitelist=None):
        ...
        if whitelist is None:
            whitelist = {}
        self.label_enum = self.GenerateEnum(code_list, LABEL, code_whitelist=whitelist)
        self.order_enum = self.GenerateEnum(code_list, ORDER, code_whitelist=whitelist)
        self.family_enum = self.GenerateEnum(
            code_list, FAMILY, code_whitelist=whitelist
        )
        self.genus_enum = self.GenerateEnum(code_list, GENUS, code_whitelist=whitelist)
    ...
    def TranslateLabelVector(self, label_vector, other_taxonomy_path):
        ...
        trans = np.zeros([taxo_old.NumLabels(), self.NumLabels()], label_vector.dtype)
        misses = []
