...
def old_generate_annotations_for_probing_task(
    ...
    **kwargs,
    ...
    if (
        ...
        inv = {
            v: k for k, v in ground_truth[f"label_dict:{label_column}"].items()
        }
        ...
        if not paths is None:
            filenames, starts = get_filenames_and_starts_for_probe_df(
                paths, ground_truth, label_column
            )
        ...
        for v in inv.values():
            ...
            te_ar = ar[
                int(l * train_ratio) : int(l * (train_ratio + test_ratio))
            ]
