...
birdnet_list = "/home/siriussound/Code/bacpipe/bacpipe/model_checkpoints/birdnet/BirdNET_GLOBAL_6K_V2.4_Labels_en_uk.txt"
MIN_FILES_PER_LABEL = 3
...
def clean_df(df, start_col, end_col, label_col, file_col):
    df = df.dropna()
    labs, cnts = np.unique(df[label_col], return_counts=True)
    df = df[get_durations(df, start_col, end_col) > 1]
...
if False:  # for AnuranSet
    ...
    anomals = clean_df(df, "start", "end", "species", "wavfilename")
...
if False:  # for neotropic katydids sounds
...
if False:  # for neotropical coffee farms
    ...
    anomals.to_csv(src / "anomals.csv")

if False:  # for dawn chorus California, Oregon, and Washington
    src = Path(
    ...
    df = clean_df(df, "start", "end", "eBird_2021", "file")
    ...
    anomals.to_csv(src.parent / "anomals.csv")

if False:  # for ArcticBirdSounds
    src = Path(
    ...
    anomals = clean_df(df, "start", "end", "species", "file_name")
    ...
    good_labels = check_min_files_per_label(
        anomals, anomals.species.unique(), "species", "file_name"
    )
...
if False:  # for WABAD
    ...
    anomals = clean_df(df, "Begin Time (s)", "End Time (s)", "Species", "Recording")
    ...
    good_labels = check_min_files_per_label(
        anomals, anomals.species.unique(), "species", "Recording"
    )
